...
- 创建 `stock_info/stock_daily/stock_scores_v3/stock_levels/model_laowang_pool/model_fhkq`。
...
```bash
python laowang.py --config config.ini --start-date 2026-01-01 --end-date 2026-01-21 --workers 32 --top 200 --min-score 60
```
...
```bash
python fhkq.py --config config.ini --start-date 2026-01-01 --end-date 2026-01-21 --workers 16
```
...
2. 调用 `laowang.py`、`fhkq.py`，评分区间覆盖“新增交易日或最新一日”。
